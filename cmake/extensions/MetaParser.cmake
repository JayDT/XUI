...
function(meta_parser_prebuild)
    ...
    set(MODULE_SOURCE "${PREBUILD_META_GENERATED_DIR}/Assembly_Modul_${PREBUILD_META_TARGET}.cpp")
    ...
    foreach (HEADER ${PREBUILD_META_HEADER_FILES})
        ...
        if (NOT "${EXTENSION}" STREQUAL ".hpp")
            file(RELATIVE_PATH RELATIVE 
                ...
                "${DIRECTORY_NAME}/Assembly_${BASE_NAME}"
            )

            #set(GENERATED_HEADER "${PREBUILD_META_GENERATED_DIR}/${RELATIVE}.h")
            set(GENERATED_SOURCE "${PREBUILD_META_GENERATED_DIR}/${RELATIVE}.cpp")
            set(GENERATED_DEF_SOURCE "${PREBUILD_META_GENERATED_DIR}/${RELATIVE}.definition.cpp")

            list(APPEND GENERATED_FILES ${GENERATED_SOURCE} ${GENERATED_DEF_SOURCE}) #${GENERATED_HEADER}
            list(APPEND GENERATED_HEADERS ${GENERATED_HEADER})
            list(APPEND GENERATED_SOURCES ${GENERATED_SOURCE} ${GENERATED_DEF_SOURCE})
        endif ()
    endforeach ()

    source_group(".Assembly" FILES ${GENERATED_FILES})
...
function(meta_parser_build)
    ...
    if (NOT "${BUILD_META_PCH_NAME}" STREQUAL "")
        ...
        set(PCH_SWITCH "--pch")
        set(BUILD_META_PCH_NAME "${BUILD_META_PCH_NAME}.h")
    else ()
    ...
    get_filename_component(WORK_DIRECTORY_EXEC ${BUILD_META_PARSER_EXECUTABLE} DIRECTORY)
    get_filename_component(EXEC_NAME ${BUILD_META_PARSER_EXECUTABLE} NAME)

    #message("OUTPUT ${BUILD_META_GENERATED_FILES}
    ...
    #    ${PCH_SWITCH} ${BUILD_META_PCH_NAME}
    ...
    add_custom_command(
        ...
        --target-name "${BUILD_META_TARGET}"
        --source-root "${BUILD_META_SOURCE_ROOT}"
        --in-source "${BUILD_META_SOURCE_FILE}"
        --module-header "${BUILD_META_SOURCE_ROOT}/${BUILD_META_MODULE_HEADER}"
        --out-source "${BUILD_META_MODULE_SOURCE_FILE}"
        --out-dir "${BUILD_META_GENERATED_DIR}"
        --includes "${INCLUDES_FILE}"
        ${PCH_SWITCH} ${BUILD_META_PCH_NAME}
        ${DEFINES_SWITCH}
        COMMENT "[MP]: Generate reflection assembly"
    )
endfunction ()