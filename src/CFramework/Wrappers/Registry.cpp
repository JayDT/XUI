...
DWORD System::Wrappers::GetRegistryUIntValue(CRegistry * RegHandle, const char * rId, DWORD defaultValue)
{
    DWORD value = 0;
    DWORD _ret = RegHandle->ReadDoubleWordValue(&value, rId);
    if (_ret == ERROR_SUCCESS)
        return value;
    return defaultValue;
}

std::string System::Wrappers::GetRegistryStringValue(CRegistry * RegHandle, const char * rId, const char * defaultValue)
{
    std::string value;
    DWORD _ret = RegHandle->ReadStringValue(value, rId);
    if (_ret == ERROR_SUCCESS)
        return value;
    return defaultValue;
}
