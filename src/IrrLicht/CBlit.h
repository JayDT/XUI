...
//! Converts a 32bit GL_BGRA_EXT
inline u32 RBG8toA8R8G8B8(u32 color)
{
    return (u32)(
        (color & 0xFF000000)       |  // a -> a
        (color & 0x00FF0000) >> 16 |  // b -> r
        (color & 0x0000FF00)       |  // g -> g
        (color & 0x000000FF) << 16);  // r -> b
}

/*!
*/
static void executeBlit_TextureCopy_RBG8_to_A8R8G8B8(const SBlitJob * job)
{
    ...
    if (job->stretch)
    {
        ...
        for (u32 dy = 0; dy < h; ++dy)
        {
            ...
            for (u32 dx = 0; dx < w; ++dx)
            {
                ...
                dst[dx] = RBG8toA8R8G8B8(src[src_x]);
            }
