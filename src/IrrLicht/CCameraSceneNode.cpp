...
CCameraSceneNode::CCameraSceneNode(ISceneNode* parent, ISceneManager* mgr, s32 id,
	...
	InputReceiverEnabled(true), TargetAndRotationAreBound(false), FirstPersonMode(true)
{
	#ifdef _DEBUG
...
//! 
void CCameraSceneNode::setFirstPersonMode(bool on)
{
    FirstPersonMode = on;
}

//! 
bool CCameraSceneNode::isFirstPerson(void) const
{
    return FirstPersonMode;
}

void CCameraSceneNode::setPosition(const core::vector3df& newpos)
{
    //ISceneNodeAnimatorList::Iterator ait = Animators.begin();
    //
    //for (; ait != Animators.end(); ++ait)
    //    (*ait)->setPosition(newpos);
