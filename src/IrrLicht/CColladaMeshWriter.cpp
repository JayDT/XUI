...
bool CColladaMeshWriter::writeScene(io::IWriteFile* file, scene::ISceneNode* root)
{
	...
	Writer->writeLineBreak();
		...
		else
		{
			...
			const ISceneNodeList& rootChildren = root->getChildren();
            for (ISceneNodeList::iterator it = rootChildren.begin();
					it != rootChildren.end();
					++ it )
			{
				writeSceneNode(*it);
...
void CColladaMeshWriter::makeMeshNames(irr::scene::ISceneNode * node)
{
    ...
    const ISceneNodeList& children = node->getChildren();
    for (ISceneNodeList::const_iterator it = children.begin(); it != children.end(); ++it)
	{
		makeMeshNames(*it);
	}
}
