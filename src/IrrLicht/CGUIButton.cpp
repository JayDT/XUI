    ...
    case EET_MOUSE_INPUT_EVENT:
        if (event.MouseInput.Event == EMIE_LMOUSE_PRESSED_DOWN)
        {
            if (Environment->hasFocus(this) &&
                !isPointInside(core::position2d<s32>(event.MouseInput.X, event.MouseInput.Y)))
            {
...
void CGUIButton::draw()
{
    ...
    driver->setTransform(video::ETS_VIEW_2D, getAbsoluteTransformation());

	auto& rect = BaseRect; // AbsoluteRect
	auto& rectClip = BaseRectClippingRect; // AbsoluteClippingRect

    // todo:	move sprite up and text down if the pressed state has a sprite
    const core::position2di spritePos = rect.getCenter();

    if (!Pressed)
    {
        if (DrawBorder)
            skin->draw3DButtonPaneStandard(this, rect, &rectClip);
    ...
    {
        if (DrawBorder)
            skin->draw3DButtonPanePressed(this, rect, &rectClip);
    ...
    if (SpriteBank)
    {
        ...
        if (ButtonSprites[state].Index != -1)
        {
            SpriteBank->draw2DSprite(ButtonSprites[state].Index, spritePos,
                &rectClip, ButtonSprites[state].Color, ClickTime, os::Timer::getTime(),
        ...
        {
            SpriteBank->draw2DSprite(ButtonSprites[state].Index, spritePos,
                &rectClip, ButtonSprites[state].Color, FocusTime, os::Timer::getTime(),
                ButtonSprites[state].Loop, true);
        }
        ...
        if (isEnabled())
        {
            ...
            {
                SpriteBank->draw2DSprite(ButtonSprites[state].Index, spritePos,
                    &rectClip, ButtonSprites[state].Color, HoverTime, os::Timer::getTime(),
                    ButtonSprites[state].Loop, true);
            }
        }
    }
    ...
    if (Text.size())
    {
        ...
        core::rect<s32> _rect = rect;
        if (Pressed)
        {
			_rect.UpperLeftCorner.X += skin->getSize(EGDS_BUTTON_PRESSED_TEXT_OFFSET_X);
			_rect.UpperLeftCorner.Y += skin->getSize(EGDS_BUTTON_PRESSED_TEXT_OFFSET_Y);
        }

        if (font)
            font->draw(Text.c_str(), _rect,
                skin->getColor(isEnabled() ? EGDC_BUTTON_TEXT : EGDC_GRAY_TEXT),
                true, true, &rectClip);
    }

    driver->setTransform(video::ETS_VIEW_2D, core::IdentityMatrix);
