...
void CGUIEditBoxIRB::operator=(const wchar_t* text)
{
    setText(text);
    sendGuiEvent(EGET_EDITBOX_CHANGED);
}

video::SColor CGUIEditBoxIRB::getOverrideColor() const
{
    return OverrideColor;
}

bool CGUIEditBoxIRB::isOverrideColorEnabled() const
{
    _IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
    return OverrideColorEnabled;
}

void CGUIEditBoxIRB::setDrawBackground(bool draw)
{
    //Background = draw;
}

bool CGUIEditBoxIRB::isDrawBackgroundEnabled() const
{
    _IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
    return false; //Background;
}

IGUIFont* CGUIEditBoxIRB::getActiveFont() const
{
    if (OverrideFont)
        return OverrideFont;
...
IGUIFont* CGUIEditBoxIRB::getOverrideFont() const
{
    return OverrideFont;
}
	...
	if (font)
	{
		...
		if (Text.size())
		{
			...
			for (s32 i=0; i < lineCount; ++i)
			{
				...
				setTextRect(i);
				while (CurrentTextRect.LowerRightCorner.Y < AbsoluteRect.UpperLeftCorner.Y && (i+1) < lineCount) {
					setTextRect(++i);
				}
				if (numberStartY == -1)
