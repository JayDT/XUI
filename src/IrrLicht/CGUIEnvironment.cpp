...
void DockedElementInfo::DockShowOrHide(void* sender, GUIEventArg& arg)
{
    if (!dockElement->isVisible() || dockElement->IsCanDockHide())
        dockElement->setVisible(!dockElement->isVisible());
    if (dockElement->isVisible())
        dockElement->GetEnvironment()->setFocus(dockElement);
}

    //! constructor
CGUIEnvironment::CGUIEnvironment(IrrlichtDevice* device, io::IFileSystem* fs, video::IVideoDriver* driver, IOSOperator* op)
: IGUIElement(EGUIET_ROOT, 0, 0, 0, core::rect<s32>(core::position2d<s32>(0,0), driver ? core::dimension2d<s32>(driver->getScreenSize()) : core::dimension2d<s32>(0,0))),
    Device(device), Driver(driver), Hovered(0), HoveredNoSubelement(0), Focus(0), LastHoveredMousePos(0,0), CurrentSkin(0),
    FileSystem(fs), UserReceiver(0), FocusFlags(EFF_SET_ON_LMOUSE_DOWN | EFF_SET_ON_TAB), Operator(op), ShowDockIcons(false)
{
    if (Driver)
        Driver->grab();
    ...
    if (m_screenSize != screenSize)
    {
        ...
        rectLeft = core::recti(
            0,
            (screenSize.Height / 2) - (sizeofMark / 2),
            sizeofMark,
            (screenSize.Height / 2) + (sizeofMark / 2));

        rectRight = core::recti(
            screenSize.Width - sizeofMark,
            (screenSize.Height / 2) - (sizeofMark / 2),
            screenSize.Width,
            (screenSize.Height / 2) + (sizeofMark / 2));

        rectTop = core::recti(
            (screenSize.Width / 2) - (sizeofMark / 2),
            0,
            (screenSize.Width / 2) + (sizeofMark / 2),
            sizeofMark);

        rectBottom = core::recti(
            (screenSize.Width / 2) - (sizeofMark / 2),
            screenSize.Height - sizeofMark,
            (screenSize.Width / 2) + (sizeofMark / 2),
            screenSize.Height);
    }
}
...
E_WINDOW_DOCK_TYPE CGUIEnvironment::getAvailableDockTypeByPoint(core::position2di const& point)
{
    if (rectLeft.isPointInside(point))
        return EWD_DOCK_LEFT;

    if (rectRight.isPointInside(point))
        return EWD_DOCK_RIGHT;

    if (rectTop.isPointInside(point))
        return EWD_DOCK_TOP;

    if (rectBottom.isPointInside(point))
        return EWD_DOCK_BOTTOM;
...
{
    io::IReadFile* file = FileSystem->createMemoryReadFile(BuiltInFontData, BuiltInFontDataSize, DefaultFontName, false);
...
void CGUIEnvironment::drawAll()
{
    if (Driver)
    {
        ...
        if (ShowDockIcons)
        {
            auto color = video::SColor(240, 30, 30, 30); //skin->getColor(EGDC_WINDOW_SYMBOL);
            Driver->draw2DRectangle(color, rectLeft);
            Driver->draw2DRectangle(color, rectRight);
            Driver->draw2DRectangle(color, rectTop);
            Driver->draw2DRectangle(color, rectBottom);

            if (skin->getSpriteBank())
            {
                // mouse over / off animation
                auto icon = rectLeft.isPointInside(GetDevice()->getCursorControl()->getPosition()) ? skin->getIcon(EGDI_WINDOW_RESTORE) : skin->getIcon(EGDI_WINDOW_MAXIMIZE);
                skin->getSpriteBank()->draw2DSprite(icon, rectLeft.getCenter(),
                    &rectLeft, video::SColor(255, 255, 255, 255), 0, os::Timer::getTime(),
                    false, true);

                // mouse over / off animation
                icon = rectRight.isPointInside(GetDevice()->getCursorControl()->getPosition()) ? skin->getIcon(EGDI_WINDOW_RESTORE) : skin->getIcon(EGDI_WINDOW_MAXIMIZE);
                skin->getSpriteBank()->draw2DSprite(icon, rectRight.getCenter(),
                    &rectRight, video::SColor(255, 255, 255, 255), 0, os::Timer::getTime(),
                    false, true);

                // mouse over / off animation
                icon = rectTop.isPointInside(GetDevice()->getCursorControl()->getPosition()) ? skin->getIcon(EGDI_WINDOW_RESTORE) : skin->getIcon(EGDI_WINDOW_MAXIMIZE);
                skin->getSpriteBank()->draw2DSprite(icon, rectTop.getCenter(),
                    &rectTop, video::SColor(255, 255, 255, 255), 0, os::Timer::getTime(),
                    false, true);

                // mouse over / off animation
                icon = rectBottom.isPointInside(GetDevice()->getCursorControl()->getPosition()) ? skin->getIcon(EGDI_WINDOW_RESTORE) : skin->getIcon(EGDI_WINDOW_MAXIMIZE);
                skin->getSpriteBank()->draw2DSprite(icon, rectBottom.getCenter(),
                    &rectBottom, video::SColor(255, 255, 255, 255), 0, os::Timer::getTime(),
                    false, true);
            }
        ...
        auto color = video::SColor(70, 100, 100, 100); //skin->getColor(EGDC_WINDOW_SYMBOL);

        if (!GUIDockedElements[EWD_DOCK_LEFT].empty())
        {
            Driver->draw2DRectangle(color, core::recti(0, 0, 20, screenSize.Height));
        }

        if (!GUIDockedElements[EWD_DOCK_RIGHT].empty())
        {
            Driver->draw2DRectangle(color, core::recti(screenSize.Width - 20, 0, screenSize.Width, screenSize.Height));
        }
    
        if (!GUIDockedElements[EWD_DOCK_TOP].empty())
        {
            Driver->draw2DRectangle(color, core::recti(0, 0, screenSize.Width, 20));
        }
    
        if (!GUIDockedElements[EWD_DOCK_BOTTOM].empty())
        {
            Driver->draw2DRectangle(color, core::recti(0, screenSize.Height - 20, screenSize.Width, screenSize.Height));
        }
    }
    ...
    if (!m_disposeElement.empty())
    {
        IReferenceCounted* diposeElement;
        if (m_disposeElement.pop(diposeElement))
        {
            diposeElement->drop();
        }
    }
...
bool CGUIEnvironment::postEventFromUser(const SEvent& event)
{
    ...
    case EET_MOUSE_INPUT_EVENT:
        ...
        {
            //if (Hovered->OnInputMousePassive)
            //{
            //    GUIEventArg arg;
            //    arg.eData = event;
            //    Hovered->OnInputMousePassive(Hovered, arg);
            //}
        }
...
//! Saves the current gui into a file.
bool CGUIEnvironment::saveGUI(io::IWriteFile* file, IGUIElement* start, bool isMinimalSerialize, bool withChildrens)
{
    if (!file)
    {
        _IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
    ...
    writeGUIElement(writer, start ? start : this, isMinimalSerialize, withChildrens);
    writer->drop();
...
//! \param filename: Name of the file.
bool CGUIEnvironment::loadGUI(const io::path& filename, IGUIElement* parent, core::array<IGUIElement*>* loaded)
{
    ...
    bool ret = loadGUI(read, parent, loaded);
    read->drop();
...
//! Loads the gui. Note that the current gui is not cleared before.
bool CGUIEnvironment::loadGUI(io::IReadFile* file, IGUIElement* parent, core::array<IGUIElement*>* loaded, IGUIElement* element)
{
    if (!file)
    {
    ...
    while(reader->read())
    {
        readGUIElement(reader, parent, loaded, element);
    }
...
//! reads an element
void CGUIEnvironment::readGUIElement(io::IXMLReader* reader, IGUIElement* node, core::array<IGUIElement*>* loaded, IGUIElement* element)
{
    if (!reader)
        return;
    ...
    else if	(!wcscmp(IRR_XML_FORMAT_GUI_ELEMENT, reader->getNodeName()))
    {
        ...
        if (element)
        {
            if (attrName != element->getTypeName())
                return;
    ...
    while(reader->read())
    {
        ...
        case io::EXN_ELEMENT:
            if (!wcscmp(L"attributes", reader->getNodeName()))
            {
                ...
                if (node && attr->getAttributeCount())
                ...
                !wcscmp(IRR_XML_FORMAT_GUI_ENV, reader->getNodeName()))
            {
                ...
                if ( deferedNode )
                    readGUIElement(reader, deferedNode, loaded);
                else
                    readGUIElement(reader, node, loaded);
            }
            else
            {
...
//! writes an element
void CGUIEnvironment::writeGUIElement(io::IXMLWriter* writer, IGUIElement* node, bool isMinimalSerialize, bool withChildrens)
{
    if (!writer || !node )
        return;
    ...
    attr->SetMinimalSerialize(isMinimalSerialize);
    ...
    if (withChildrens)
    {
        ...
        {
            if (!(*it)->isSubElement())
                writeGUIElement(writer, (*it), isMinimalSerialize);
        }
    }
...
IGUIFileOpenDialog* CGUIEnvironment::addFileSelectorDialog(IrrlichtDevice* device, const wchar_t* title, bool modal, IGUIElement* parent, s32 id, bool isSaveDialog, bool restoreCWD, io::path::char_type* startDir)
{
    parent = parent ? parent : this;

    CGUIFileSelector* d = new CGUIFileSelector(title, this, parent, id, isSaveDialog ? EFST_SAVE_DIALOG : EFST_OPEN_DIALOG, restoreCWD, startDir);
    d->setDevice(GetDevice());
    d->drop();
...
//! Adds an edit box. The returned pointer must not be dropped.
IGUIEditBox* CGUIEnvironment::addCodeEditBox(const wchar_t* text, const core::rect<s32>& rectangle, bool border, bool lines,
    IGUIElement* parent, s32 id, IrrlichtDevice* Device)
{
    IGUIEditBox* d = new CGUIEditBoxIRB(text, border, lines, this,
        parent ? parent : this, id, rectangle, Device);
...
//! returns the font
IGUIFont* CGUIEnvironment::getFont(const io::path& filename, s32 fontSize /*= -1*/, bool UseFreeTypeFont /*= false*/)
{
    ...
    if (UseFreeTypeFont)
    {
        ...
        irr::core::stringc newname = f.NamedPath;
        newname = newname.append(std::to_string(fontSize).c_str());
        f.NamedPath.setPath(newname);
    }
    ...
    if (UseFreeTypeFont)
    {
        ifont = static_cast<IGUIFont*>(CGUITTFont::create(this, filename, fontSize));
    }
    else
    {
...
IGUIElement* CGUIEnvironment::getElementFromPoint(const core::vector2d<s32>& point, IGUIElement* except /*= nullptr*/)
{
    ...
    if (isVisible())
    {
        //for (int i = 0; i != EWD_MAX_DOCK_TYPE; ++i)
        //{
        //    for (auto dockedButton : GUIDockedElements[i])
        //    {
        //        if (dockedButton.second.dockButton->isVisible() && dockedButton.second.dockButton->isPointInside(point))
        //            return dockedButton.second.dockButton;
        //        if (dockedButton.second.dockElement->isVisible() && dockedButton.second.dockElement->isPointInside(point))
        //        {
        //            auto it = dockedButton.second.dockElement->getChildren().getLast();
        //
        //            while (it != dockedButton.second.dockElement->getChildren().end())
        //            {
        ...
        //            }
        //
        //            return dockedButton.second.dockElement;
        //        }
        //    }
        //}
        ...
        while (it != Children.end())
        {
            ...
            if (target && getRootGUIElement()->getAbsolutePosition() != target->getAbsolutePosition())
                return target;
...
void CGUIEnvironment::DisposeElement(gui::IGUIElement* element)
{
    m_disposeElement.push(element);
}

void CGUIEnvironment::DisposeForm(irr::Forms::IForm* form)
{
    m_disposeElement.push(form);
}

//! creates an GUI Environment
IGUIEnvironment* createGUIEnvironment(IrrlichtDevice* device, io::IFileSystem* fs,
                    video::IVideoDriver* Driver,
                    IOSOperator* op)
{
    return new CGUIEnvironment(device, fs, Driver, op);
}
