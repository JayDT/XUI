...
class IRRLICHT_API CGUIEnvironment : public IGUIEnvironment, public IGUIElement
{
public:

	//! constructor
	CGUIEnvironment(IrrlichtDevice* device, io::IFileSystem* fs, video::IVideoDriver* driver, IOSOperator* op);

	//! destructor
	virtual ~CGUIEnvironment();

    virtual E_WINDOW_DOCK_TYPE getAvailableDockTypeByPoint(core::position2di const&) _IRR_OVERRIDE_;
    virtual void DockGUIElement(E_WINDOW_DOCK_TYPE dockType, IGUIElement* guielement);
    virtual bool DockByPoint(const core::position2d<s32>& point, IGUIElement* dockElement)
    {
        return IGUIElement::DockByPoint(point, dockElement);
    }

    virtual void showDockIcons(bool on);
	...
	//! returns the font
	virtual IGUIFont* getFont(const io::path& filename, s32 fontSize = -1, bool UseFreeTypeFont = false);
    ...
    virtual IGUIFileOpenDialog* addFileSelectorDialog(IrrlichtDevice* device, const wchar_t* title = 0,
        bool modal = true, IGUIElement* parent = 0, s32 id = -1, bool isSaveDialog = false,
        bool restoreCWD = false, io::path::char_type* startDir = 0);
    ...
    //! Adds an edit box. The returned pointer must not be dropped.
    virtual IGUIEditBox* addCodeEditBox(const wchar_t* text, const core::rect<s32>& rectangle, bool border, bool lines,
        IGUIElement* parent = 0, s32 id = -1, IrrlichtDevice* Device = nullptr);
	...
	\param start: The element to start saving from.
	if not specified, the root element will be used */
	virtual bool saveGUI(io::IWriteFile* file, IGUIElement* start=0, bool isMinimalSerialize = false, bool withChildrens = true);

	//! Loads the gui. Note that the current gui is not cleared before.
	/** \param filename: Name of the file.
	\param parent: The parent of all loaded GUI elements,
	if not specified, the root element will be used */
	virtual bool loadGUI(const io::path& filename, IGUIElement* parent=0, core::array<IGUIElement*>* loaded = nullptr);

	//! Loads the gui. Note that the current gui is not cleared before.
	/** \param file: IReadFile to load the GUI from
	\param parent: The parent of all loaded GUI elements,
	if not specified, the root element will be used */
	virtual bool loadGUI(io::IReadFile* file, IGUIElement* parent=0, core::array<IGUIElement*>* loaded = nullptr, IGUIElement* element = nullptr);
	...
	//! writes an element
	virtual void writeGUIElement(io::IXMLWriter* writer, IGUIElement* node, bool isMinimalSerialize = false, bool withChildrens = true);

	//! reads an element
	virtual void readGUIElement(io::IXMLReader* reader, IGUIElement* node, core::array<IGUIElement*>* loaded = nullptr, IGUIElement* element = nullptr);
    ...
    virtual void DisposeElement(gui::IGUIElement* element) _IRR_OVERRIDE_;
    virtual void DisposeForm(irr::Forms::IForm* element) _IRR_OVERRIDE_;
private:
    ...
    std::unordered_map<IGUIElement*, DockedElementInfo> GUIDockedElements[EWD_MAX_DOCK_TYPE];

    core::recti rectLeft;
    core::recti rectRight;
    core::recti rectTop;
    core::recti rectBottom;
