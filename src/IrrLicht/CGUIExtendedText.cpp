...
void CGUIExtendedText::appendText( const wchar_t* text )
{
    ...
    dim = CurrentFont->getDimension(text);
    appendText( text, dim);
}
...
void CGUIExtendedText::parseText( const wchar_t* text )
{
    ...
    core::dimension2di squareDim;
    squareDim = CurrentFont->getDimension(L"#");
    ...
    while ( *c != 0 )
    {
        ...
        {
            core::dimension2di Dim;
            Dim = CurrentFont->getDimension(str.c_str());
            appendText( str.c_str(), Dim);
        }
