...
core::dimension2d<u32> CGUIFont::MeasureText(const wchar_t * text, irr::core::recti & bounds)
{
    return getDimension(text);
}

core::dimension2d<u32> CGUIFont::MeasureText(const wchar_t * text)
{
    return getDimension(text);
}

core::dimension2d<u32> CGUIFont::MeasureText(const wchar_t * text, u32 length)
{
...
u32 CGUIFont::BreakText(const wchar_t * text, float maxWidth, float * measuredWith) const
{
    ...
    if (text)
    {
        for (const wchar_t* p = text; *p; ++p)
        {
            ...
            const SFontArea &area = Areas[getAreaFromCharacter(*p)];
            if ((area.width + area.underhang + _measuredWith) >= maxWidth)
            {
                curPos = (intptr_t(p) - intptr_t(text)) / sizeof(wchar_t);
                break;
            }

            _measuredWith += area.underhang;
            _measuredWith += area.width + area.overhang + GlobalKerningWidth;
        }
    }
                ...
                )
{
    ...
    if (!cache || cache->GetText() != text)
    {
        ...
        if (cache)
        {
            cache->GetBuffer().Set3DMode(false);
            SpriteBank->CacheGeometry(cache->GetBuffer(), indices, offsets, clip, color);
        }
        else
    ...
    if (cache)
    {
        Driver->setMaterial(cache->GetBuffer().getMaterial());
        Driver->drawMeshBuffer(&cache->GetBuffer());
        if (cache->GetBuffer().GetHWBuffer())
            cache->GetBuffer().GetHWBuffer()->Unbind();
    }
}
