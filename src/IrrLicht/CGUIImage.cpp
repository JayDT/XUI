    ...
    if (Texture)
    {
        if (ScaleImage)
        {
            ...
            driver->draw2DImage(Texture, rect,
                ...
                &rectClip, Colors, UseAlphaChannel);
        }
        else
        {
            driver->draw2DImage(Texture, rect.UpperLeftCorner,
                core::rect<s32>(core::position2d<s32>(0,0), core::dimension2di(Texture->getOriginalSize())),
                &rectClip, Color, UseAlphaChannel);
        }
    }
    else
    {
        skin->draw2DRectangle(this, skin->getColor(EGDC_3D_DARK_SHADOW), rect, &rectClip);
    }
...
bool CGUIImage::OnEvent(const irr::SEvent& event)
{
    //if (isEnabled())
    //{
    //
