...
: IGUIListBox(environment, parent, id, rectangle), Selected(-1),
    ItemHeight(0), HighestItemLineCnt(1), LastHighestItemLineCnt(1),ItemHeightOverride(0),
...
void CGUIListBox::recalculateItemHeight()
{
    ...
    if (Font != skin->getFont() || LastHighestItemLineCnt != HighestItemLineCnt)
    {
        if (Font)
            Font->drop();
        ...
        if (Font)
        {
            ...
            ItemHeight *= HighestItemLineCnt;
            Font->grab();
        }

        LastHighestItemLineCnt = HighestItemLineCnt;
    }
...
u32 CGUIListBox::addItem(const wchar_t* text, s32 icon)
{
    ...
    i.text = text;
    i.icon = icon;
    i.lineCnt = 1 + System::String(text).Count('\n');
    HighestItemLineCnt = std::max(HighestItemLineCnt, (s8)i.lineCnt);
