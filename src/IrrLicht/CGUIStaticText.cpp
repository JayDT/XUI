		...
		if (font)
		{
			if (!WordWrap)
			{
				...
				font->draw(Text.c_str(), frameRect,
					...
					HAlign == EGUIA_CENTER, VAlign == EGUIA_CENTER, (RestrainTextInside ? &AbsoluteClippingRect : NULL), &mFontCache);
			}
			else
			{
				...
				for (u32 i=0; i<BrokenText.size(); ++i)
				{
					...
					font->draw(BrokenText[i].c_str(), r,
						OverrideColorEnabled ? OverrideColor : skin->getColor(isEnabled() ? EGDC_BUTTON_TEXT : EGDC_GRAY_TEXT),
						HAlign == EGUIA_CENTER, false, (RestrainTextInside ? &AbsoluteClippingRect : NULL), &mFontCache);
