...
void CGUITTFont::setMaxHeight()
{
    ...
    for (FT_Long i = 0; i < tt_face->num_glyphs; ++i)
    {
        t = Glyphs[i].source_rect.getHeight();
        if (t>int(maxHeight))
            maxHeight = t;
    }
}
...
void CGUITTFont::draw(const core::stringw& text, const core::rect<s32>& position, video::SColor color, bool hcenter, bool vcenter, const core::rect<s32>* clip, IGUIFontCache * cache /*= nullptr*/)
{
	draw(EnrichedString(std::wstring(text.c_str()), color), position, color, hcenter, vcenter, clip);
}

void CGUITTFont::draw(const EnrichedString &text, const core::rect<s32>& position, video::SColor color, bool hcenter, bool vcenter, const core::rect<s32>* clip, IGUIFontCache * cache /*= nullptr*/)
{
	...
	while (!iter.atEnd())
	{
		...
		if (n > 0 && visible)
		{
			...
			s32 offy = maxHeight - Glyphs[n-1].offset.Y;
...
{
	core::dimension2d<u32> text_dimension(0, maxHeight);
	core::dimension2d<u32> line(0, maxHeight);
...
u32 CGUITTFont::BreakText(const wchar_t * text, float maxWidth, float * measuredWith /*= nullptr*/) const
{
    float _measuredWith = 0.f;
    int curPos = 0;
    ...
    if (text)
    {
        for (const wchar_t* p = text; *p; ++p)
        {
            ...
            if (lineBreak)
            {
                curPos = (intptr_t(p) - intptr_t(text)) / sizeof(wchar_t);
                break;
            }
            ...
            size_t widthChar = getWidthFromCharacter(*p);
            if ((widthChar + _measuredWith) >= maxWidth)
            {
                curPos = (intptr_t(p) - intptr_t(text)) / sizeof(wchar_t);
                break;
            }

            _measuredWith += widthChar + k.X + GlobalKerningWidth;
        }
    }
...
core::dimension2d<u32> CGUITTFont::MeasureText(const wchar_t * text, irr::core::recti & bounds)
{
    return getDimension(text);
}

core::dimension2d<u32> CGUITTFont::MeasureText(const wchar_t * text)
{
    return getDimension(text);
}

core::dimension2d<u32> CGUITTFont::MeasureText(const wchar_t * text, u32 length)
{
    ...
    if (text)
    {
        for (u32 i = 0; i < length; ++i)
        {
            ...
            size_t widthChar = getWidthFromCharacter(*p);
            thisLine.Width += widthChar + k.X + GlobalKerningWidth;
        }
    }
