...
void CGUITab::StartGrabbing()
{
    OnGrabbing = true;
}

bool CGUITab::OnEvent(const SEvent& event)
{
...
CGUITabControl::CGUITabControl(IGUIEnvironment* environment,
    ...
    Border(border), FillBackground(fillbackground), ScrollControl(false), CanCreateNewWindow(true), TabHeight(0), VerticalAlignment(EGUIA_UPPERLEFT),
    UpButton(0), DownButton(0), TabMaxWidth(0), CurrentScrollTabIndex(0), TabExtraWidth(20)
    , GrabbingTab(-1)
    , GrabbingTabLastPosition(-1)
{
    #ifdef _DEBUG
...
bool CGUITabControl::insertTabByPostion(core::vector2di const& pos, IGUITab* tab)
{
    s32 hoveredID = getTabAt(pos.X, pos.Y);
    if (hoveredID == -1)
        return false;
    ...
    if (hoveredID == tab->getNumber())
    {
        GrabbingTabLastPosition = -1;
        return false;
    }

    if (GrabbingTabLastPosition == hoveredID)
        return false;

    tab->grab();
    removeTab(tab->getNumber());

    Tabs.insert(tab, (u32)hoveredID);
    
    if (GrabbingTab != -1)
    {
        GrabbingTabLastPosition = tab->getNumber();
        GrabbingTab = hoveredID;
    }

    tab->setNumber(hoveredID);

    for (u32 i = (u32)hoveredID + 1; i < Tabs.size(); ++i)
    {
        Tabs[i]->setNumber(i);
    }
    ...
    if (isEnabled())
    {
        ...
        case EET_MOUSE_INPUT_EVENT:
            ...
            case EMIE_LMOUSE_PRESSED_DOWN:
            {
                ...
                GrabbingTab = getTabAt(event.MouseInput.X, event.MouseInput.Y);
                if (GrabbingTab != -1)
                {
                    GrabbingTabLastPosition = -1;
                    auto Tab = getTab(GrabbingTab);
                    Tab->StartGrabbing();
                    setActiveTab(GrabbingTab);
                }
            ...
            case EMIE_MOUSE_MOVED:
            {
                if (GrabbingTab != -1)
                {
                    auto GrabTab = getTab(GrabbingTab);
                    if (insertTabByPostion(core::vector2di(event.MouseInput.X, event.MouseInput.Y), GrabTab))
                    {
                    }
                    else
                    {
                        auto mousePoint = core::vector2di(event.MouseInput.X, event.MouseInput.Y);
                        if (!CanCreateNewWindow || getAbsoluteClippingRect().isPointInside(mousePoint))
                            break;

                        auto GrabTab = getTab(GrabbingTab);
                        GrabTab->grab();
                        removeTab(GrabbingTab);

                        GrabbingTab = -1; // stop grabbing

                        auto TabRect = GrabTab->getAbsolutePosition();
                        
                        auto TabWindow = Environment->addWindow(TabRect, false, GrabTab->getText(), Environment->getRootGUIElement());
                        TabWindow->addChild(GrabTab);
                        //IGUIElement* optimalParent = Environment->getRootGUIElement()->getElementFromPoint(mousePoint);
                        //if (optimalParent)
                        //    optimalParent->addChild(TabWindow);

                        Environment->setFocus(TabWindow);
                        TabWindow->setRelativePosition(mousePoint);
                        TabWindow->updateAbsolutePosition();
                        TabWindow->DoDragging();
                        TabWindow->setSubElement(true);
                    }
                }
                break;
            }
