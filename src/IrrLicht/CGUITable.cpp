...
: IGUITable(environment, parent, id, rectangle), OverrideFont(0),
...
void CGUITable::setCellText(u32 rowIndex, u32 columnIndex, const core::stringw& text)
{
    ...
    {
        if (Columns[columnIndex].NumericOrder)
        {
            Rows[rowIndex].Items[columnIndex].uNumber = std::stoull(text.c_str()); // throw if invalid value
        }
...
void CGUITable::recalculateHeights()
{
    ...
    //	if(getActiveFont())
    //	{
    //		ItemHeight = getActiveFont()->getDimension(L"A").Height + (CellHeightPadding * 2);
    //        getActiveFont()->grab();
    //	}
    //}
    //TotalItemHeight = ItemHeight * Rows.size();		//  header is not counted, because we only want items
...
void CGUITable::updateAbsolutePosition()
{
    refreshControls();
}
...
void CGUITable::setNumericColumn(u32 columnIndex, bool isNumeric)
{
    if (columnIndex < Columns.size())
        Columns[columnIndex].NumericOrder = isNumeric;
}
...
void CGUITable::orderRows(s32 columnIndex, EGUI_ORDERING_MODE mode)
{
    ...
    bool numeric = Columns[columnIndex].NumericOrder;
    if ( mode == EGOM_ASCENDING )
    {
        for ( s32 i = 0 ; i < s32(Rows.size()) - 1 ; ++i )
        {
            ...
            {
                if ((!numeric && Rows[j+1].Items[columnIndex].Text < Rows[j].Items[columnIndex].Text ) ||
                    (numeric && Rows[j + 1].Items[columnIndex].uNumber < Rows[j].Items[columnIndex].uNumber))
                {
                    swap = Rows[j];
                    Rows[j] = Rows[j+1];
                    Rows[j+1] = swap;
    ...
    else if ( mode == EGOM_DESCENDING )
    {
        for ( s32 i = 0 ; i < s32(Rows.size()) - 1 ; ++i )
        {
            for ( s32 j = 0 ; j < s32(Rows.size()) - i - 1 ; ++j )
            {
                if ((!numeric && Rows[j].Items[columnIndex].Text < Rows[j + 1].Items[columnIndex].Text) ||
                    (numeric && Rows[j].Items[columnIndex].uNumber < Rows[j + 1].Items[columnIndex].uNumber))
                {
                    swap = Rows[j];
                    Rows[j] = Rows[j+1];
                    Rows[j+1] = swap;
...
void CGUITable::serializeAttributes(io::IAttributes* out, io::SAttributeReadWriteOptions* options) const
{
    ...
    for (i = 0; i < Columns.size(); ++i)
    {
        ...
        label = "Column"; label += i; label += "NumericOrder";
        out->addBool(label.c_str(), Columns[i].NumericOrder);
    }

    if (!out->IsMinimalSerialize())
    {
...
void CGUITable::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options)
{
    ...
    for (i=0;i<columnCount; ++i)
    {
        ...
        label = "Column"; label += i; label += "NumericOrder";
        column.NumericOrder = in->getAttributeAsBool(label.c_str());
