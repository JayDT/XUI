...
void CImage::initData()
{
	...
	setDebugName("CImage");
#endif
	BytesPerPixel = (f32)getBitsPerPixelFromFormat(Format) / 8.f;
	...
	if (!Data)
	{
        ...
        memset(Data, 0, Size.Height * Pitch * sizeof(u8));
	}
}
...
f32 CImage::getBitsPerPixel() const
{
...
f32 CImage::getBytesPerPixel() const
{
...
/*
u32 secretsNothingToSeeHere::getRowPitch(const FileData& _file, u32 _level)
{
	if(_file.sourceFormat == FileData::FORMAT_TGA)
	{
		elAssert(_level == 0, "Invalid mip level");
		return _file.width * _file.bytesPerPixel;
	}

	// Determine image width
	const DDS_HEADER& header = *(const DDS_HEADER*)(_file.pBuffer+4);
	u32 w = header.dwWidth;
	for(u32 i=0; i<_level; ++i)
		w >>= 1;
	if(w == 0)
		w = 1;

	const DDS_PIXELFORMAT& pf = header.ddpf;
	if((pf.dwFlags & DDPF_FOURCC) && pf.dwFourCC=='1TXD')
	{
		return max(1, ((w+3)/4)) * 8;
	}
	else if((pf.dwFlags & DDPF_FOURCC) && (pf.dwFourCC=='2TXD' || pf.dwFourCC=='3TXD' ||
		pf.dwFourCC=='4TXD' || pf.dwFourCC=='5TXD'))
	{
		return max(1, ((w+3)/4)) * 16;
	}

	return w * _file.bytesPerPixel;
}
*/
...
void CImage::copyToScaling(void* target, u32 width, u32 height, ECOLOR_FORMAT format, u32 pitch)
{
	...
	const f32 bpp=getBitsPerPixelFromFormat(format)/8.f;
	if (0==pitch)
		pitch = width*bpp;

	if ((Format==format || (Format == ECF_RGBA8 && format== ECF_A8R8G8B8)) && Size.Width==width && Size.Height==height)
	{
		if (pitch==Pitch)
		{
	...
	for (u32 y=0; y<height; ++y)
	{
		...
		{
            s32 addval = ((s32)((s32)sx)*BytesPerPixel);
			CColorConverter::convert_viaFormat(Data+ syval + addval, Format, 1, ((u8*)target)+ yval + (u32)(x*bpp), format);
			sx+=sourceXStep;
		}
		sy+=sourceYStep;
		syval=((s32)sy)*Pitch;
