    ...
    CreationParams(params), Close(false)
{
    ...
    if (os::Printer::Logger)
    {
        ...
        Logger->setConsole(ConsoleReceiver);
    }
    else
    {
...
void CIrrDeviceStub::createGUIAndScene()
{
    ...
    GUIEnvironment = gui::createGUIEnvironment(this, FileSystem, VideoDriver, Operator);
    #endif
...
void CIrrDeviceStub::addExternalSceneManager(scene::ISceneManager* _sceneManager)
{
    // drop old
    if (SceneManager)
        SceneManager->drop();
...
bool CIrrDeviceStub::postEventFromUser(const SEvent& event)
{
    ...
    if (!absorbed && ConsoleReceiver)
        absorbed = ConsoleReceiver->OnPostEvent(event);
...
void CIrrDeviceStub::setWorldEventReceiver(IEventReceiver* receiver)
{
    WorldReceiver = receiver;
}

void CIrrDeviceStub::setConsoleEventReceiver(IEventReceiver * receiver)
{
