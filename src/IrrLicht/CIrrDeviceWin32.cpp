...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SYSCHAR:
    {
        event.EventType = irr::EET_TEXT_INPUT_EVENT;
        event.KeyInput.Char = (wchar_t)wParam;
        event.KeyInput.Key = (irr::EKEY_CODE)0;
        event.KeyInput.RepeatCount = (irr::u32)lParam;
        event.KeyInput.PressedDown = false;
        event.KeyInput.SystemKey = false;
    ...
    case WM_TIMER:
        event.EventType = irr::EET_TIMER_EVENT;
        event.TimerEvent.TimerID = (irr::u32)wParam;
        event.TimerEvent.Param = (irr::s32)lParam;
...
: CIrrDeviceStub(params), HWnd(0), ChangedToFullScreen(false), Resized(false),
    ExternalWindow(false), Visible(false), Win32CursorControl(0), JoyControl(0)
{
    #ifdef _DEBUG
    ...
    if (!CreationParams.WindowId && CreationParams.DriverType != video::EDT_NULL)
    {
        ...
        if (!CreationParams.WStyle || CreationParams.WStyle & SIrrlichtCreationParameters::WS_Normal)
        {
            style |= WS_POPUP;

            if (!CreationParams.Fullscreen)
                style |= WS_SYSMENU | WS_BORDER | WS_CAPTION | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_MINIMIZEBOX | WS_MAXIMIZEBOX;
        }
        else
        {
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Popup)
            {
                style |= WS_POPUP;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Overlapped)
            {
                style |= WS_OVERLAPPED;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Dialog)
            {
                style |= WS_DLGFRAME;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Caption)
            {
                style |= WS_CAPTION;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_StrongBorder)
            {
                style |= WS_DLGFRAME;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Minimize)
            {
                style |= WS_MINIMIZE;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Maximize)
            {
                style |= WS_MAXIMIZE;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Sysmenu)
            {
                style |= WS_SYSMENU;
            }
            if (CreationParams.WStyle & SIrrlichtCreationParameters::WS_Border)
            {
                style |= WS_BORDER;
            }
...
void CIrrDeviceWin32::setWindowBackgroundLayer(irr::video::SColor const& color)
{
    //if ((CreationParams.WStyle & (SIrrlichtCreationParameters::WS_Caption | CreationParams.WStyle & SIrrlichtCreationParameters::WS_Minimize | SIrrlichtCreationParameters::WS_Maximize | SIrrlichtCreationParameters::WS_Sysmenu)) == 0)
    //{
        LONG lExStyle = GetWindowLong(HWnd, GWL_EXSTYLE);
        //lExStyle &= ~(WS_EX_DLGMODALFRAME | WS_EX_CLIENTEDGE | WS_EX_STATICEDGE);
        lExStyle |= WS_EX_LAYERED;
        SetWindowLong(HWnd, GWL_EXSTYLE, lExStyle);
    //    //SetWindowLong(HWnd, GWL_EXSTYLE, 0); //remove all window styles, check MSDN for details
    ...
    /*Use pointer to function*/
        ...
        SetLayeredWindowAttributes(HWnd, RGB(0x0, 0x0, 0x0), color.getAlpha(), LWA_ALPHA);

    //COLORREF rcol;
    //rcol = color.color; color.getAlpha() LWA_ALPHA
    //SetLayeredWindowAttributes(HWnd, RGB(0xFF, 0xFF, 0xFF), 0x10, LWA_COLORKEY);
}

void CIrrDeviceWin32::focus() const
{
    SetFocus(HWnd);
}

void CIrrDeviceWin32::unfocus() const
{
    //releasefocus
}

void CIrrDeviceWin32::capture() const
{
    SetCapture(HWnd);
}

void CIrrDeviceWin32::uncapture() const
{
    ReleaseCapture();
}

void CIrrDeviceWin32::setActiveWindow() const
{
    SetActiveWindow(HWnd);
}
...
irr::core::dimension2d<u32> CIrrDeviceWin32::getWindowSize() const
{
    RECT r;
    GetWindowRect(HWnd, &r); 
    return irr::core::dimension2d<u32>(r.right - r.left, r.bottom - r.top);
}

irr::core::dimension2d<u32> CIrrDeviceWin32::getClientSize() const
{
    RECT r;
...
void CIrrDeviceWin32::setClientSize(irr::core::dimension2d<u32> const& size) const
{
    ...
    MoveWindow(HWnd, rcWind.left, rcWind.top, size.Width + ptDiff.x, size.Height + ptDiff.y, TRUE);
}

void CIrrDeviceWin32::setWindowPosition(irr::core::position2di const& pos)
{
    SetWindowPos(HWnd, 0, pos.X, pos.Y, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
}

void CIrrDeviceWin32::invalidate(irr::core::recti const& rect)
{
    RECT _rect;
    _rect.left = rect.UpperLeftCorner.X;
    _rect.top = rect.UpperLeftCorner.Y;
    _rect.right = rect.LowerRightCorner.X;
    _rect.bottom = rect.LowerRightCorner.Y;
...
{
    auto timer = m_activeTimers.find(id);
    if (timer != m_activeTimers.end())
    {
...
void CIrrDeviceWin32::signal(s32 id, s32 param)
{
    PostMessage(HWnd, WM_USER, id, param);
}
...
void CIrrDeviceWin32::resizeIfNecessary()
{
    ...
    else
    {
        ...
        if (OnResizeFn)
            OnResizeFn(irr::core::dimension2du((u32)r.right, (u32)r.bottom));
    }
...
HCURSOR CIrrDeviceWin32::TextureToCursor(HWND hwnd, irr::video::ITexture * tex, const core::rect<s32>& sourceRect, const core::position2d<s32> &hotspot)
{
    ...
    f32 bytesPerPixel = video::IImage::getBitsPerPixelFromFormat(format) / 8.f;
...
irr::core::vector2df CIrrDeviceWin32::GetPostionToScreen(irr::core::vector2df const& relpos)
{
    HWND hWnd = getHWndFromDevice(this);
    POINT p;
    p.x = relpos.X; p.y = relpos.Y;
    ClientToScreen(hWnd, &p);
    return irr::core::vector2df(p.x, p.y);
}

irr::core::vector2df CIrrDeviceWin32::GetPostionToClient(irr::core::vector2df const& abspos)
{
    HWND hWnd = getHWndFromDevice(this);
    POINT p;
    p.x = abspos.X; p.y = abspos.Y;
