...
namespace irr
{
	...
	CLogger::CLogger(IEventReceiver* r)
		: LogLevel(ELL_INFORMATION), Receiver(r), Console(nullptr)
	{
		#ifdef _DEBUG
	...
	void CLogger::log(const c8* text, ELOG_LEVEL ll)
	{
		...
		if (Receiver || Console)
		{
            ...
            std::lock_guard<std::recursive_mutex> g(sLock);
            ...
            if (Console && Console->OnEvent(event))
                return;
    ...
    //! Sets a new event receiver
    void CLogger::setConsole(IEventReceiver* r)
    {
        Console = r;
    }
} // end namespace irr
