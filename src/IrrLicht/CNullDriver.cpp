...
void CNullDriver::removeTexture(ITexture* texture)
{
    ...
    mTextureLock.lock();
    auto iTex = Textures.find(texture->getName().getPath().c_str());
    if (iTex != Textures.end())
    {
        iTex->second->drop();
        Textures.erase(iTex);
    }
...
ITexture* CNullDriver::getTextureByIndex(u32 i)
{
    if (i < Textures.size())
    {
        auto itTexture = Textures.begin();
        std::advance(itTexture, i);
...
void CNullDriver::renameTexture(ITexture* texture, const io::path& newName)
{
    ...
    if ( iTex != Textures.end() )
    {
        ...
        Textures.erase(iTex);
        Textures.insert(std::make_pair(std::string(texture->getName().getPath().c_str()), texture));
    }
    mTextureLock.unlock();
...
video::ITexture* CNullDriver::loadTextureFromFile(io::IReadFile* file, const io::path& hashName )
{
    ...
    if (image)
    {
        ...
        texture = createDeviceDependentTexture(image, hashName.size() ? hashName : file->getFileName(), mipmapData);
...
void CNullDriver::addTexture(video::ITexture* texture)
{
    if (texture)
    {
        ...
        //s.Surface = texture;
        texture->grab();
        texture->clearImage(); // free image shadow copy

        if (!findTexture(texture->getName()))
        {
            mTextureLock.lock();
            auto& Tex = Textures[texture->getName().getPath().c_str()];
            if ( !Tex )
            {
                Tex = texture;
            }
...
video::ITexture* CNullDriver::findTexture(const io::path& filename)
{
    ...
    mTextureLock.lock();
    auto iTex = Textures.find(filename.c_str());
    mTextureLock.unlock();
    if ( iTex != Textures.end())
        return iTex->second;
...
//! Draw a 2d rectangle
void CNullDriver::draw2DRectangle(SColor color, const core::rect<s32>& pos, const core::rect<s32>* clip, bool filled /*= true*/)
{
...
IImage* CNullDriver::createImage(ITexture* texture, const core::position2d<s32>& pos, const core::dimension2d<u32>& size)
{
    ...
    else
    {
        ...
        src += clamped.UpperLeftCorner.Y * texture->getPitch() + (int)image->getBytesPerPixel() * clamped.UpperLeftCorner.X;
...
void CNullDriver::CreateHardwareBuffer(const scene::IMeshBuffer* mb)
{
    IHardwareBuffer *HWBuffer = mb->GetHWBuffer();

    if (!HWBuffer)
    {
        HWBuffer = getBufferLink(mb);
        mb->SetHWBuffer(HWBuffer);
...
IHardwareBuffer *CNullDriver::getBufferLink(const scene::IMeshBuffer* mb)
{
    if (!mb || !isHardwareBufferRecommend(mb))
        return 0;

    //search for hardware links
    //core::map< const scene::IMeshBuffer*,IHardwareBuffer* >::Node* node = HWBufferMap.find(mb);
    //if (node)
...
{
    //core::map<const scene::IMeshBuffer*,IHardwareBuffer*>::ParentFirstIterator Iterator=HWBufferMap.getParentFirstIterator();
    //
    //for (;!Iterator.atEnd();Iterator++)
    //{
    //	IHardwareBuffer *Link=Iterator.getNode()->getValue();
    //
    //	Link->LastUsed++;
...
void CNullDriver::deleteHardwareBuffer(IHardwareBuffer *HWBuffer)
{
    if (!HWBuffer)
        return;
...
void CNullDriver::removeHardwareBuffer(const scene::IMeshBuffer* mb)
{
    if (mb->GetHWBuffer())
    {
        deleteHardwareBuffer(mb->GetHWBuffer());
        mb->SetHWBuffer(nullptr);
    }
}
...
bool CNullDriver::isHardwareBufferRecommend(const scene::IMeshBuffer* mb)
{
    ...
    if (!mb->IsForceVBO() && mb->getVertexCount()<MinVertexCountForVBO)
        return false;
