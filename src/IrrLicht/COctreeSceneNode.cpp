...
COctreeSceneNode::~COctreeSceneNode()
{
    ...
    for (u32 i = 0; i != LightMapMeshes.size(); ++i)
        LightMapMeshes[i]->drop();
    LightMapMeshes.clear();

    for (u32 i = 0; i != TangentsMeshes.size(); ++i)
        TangentsMeshes[i]->drop();
...
bool COctreeSceneNode::createTree(IMesh* mesh)
{
		...
		case video::EVT_STANDARD:
			{
				...
				for (i=0; i<mesh->getMeshBufferCount(); ++i)
				{
					...
					if (b->getVertexCount() && b->getIndexCount())
					{
						...
						StdMeshes.push_back(new Octree<video::S3DVertex>::SMeshChunk());
		...
		case video::EVT_2TCOORDS:
			{
				...
				for ( i=0; i < mesh->getMeshBufferCount(); ++i)
				{
					...
					{
						Materials.push_back(b->getMaterial());
						LightMapMeshes.push_back(new Octree<video::S3DVertex2TCoords>::SMeshChunk());
		...
		case video::EVT_TANGENTS:
			{
				...
				for (u32 i=0; i<mesh->getMeshBufferCount(); ++i)
				{
					...
					{
						Materials.push_back(b->getMaterial());
						TangentsMeshes.push_back(new Octree<video::S3DVertexTangents>::SMeshChunk());
	...
	c8 tmp[255];
	snprintf(tmp, 255, "Needed %ums to create Octree SceneNode.(%u nodes, %u polys)",
