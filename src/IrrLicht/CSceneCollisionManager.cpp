...
bool CSceneCollisionManager::Intersect(G3D::Ray const& ray, scene::IIntersectionCallback* cb, ISceneNode * collisionRootNode, s32 idBitMask)
{
    if (0 == collisionRootNode)
        collisionRootNode = SceneManager->getRootSceneNode();

    const ISceneNodeList& children = collisionRootNode->getChildren();

    bool hit = false;
    float distance = cb->LastHitDistance();
    ...
    for (; it != children.end(); ++it)
    {
        ...
        if (selector && current->isVisible() && cb->m_usedNode.find(current) == cb->m_usedNode.end() &&
            (cb->getEnableFlags() & ECEF_ENABLE_DEBUG_SCENE ? !current->isDebugObject() : true) &&
            (idBitMask == 0 || (idBitMask != 0 && (current->getID() & idBitMask))))
        {
            if (selector->IntersectWithRay(ray, distance))
            {
                cb->DoHit(distance, current);
                distance = cb->LastHitDistance();
                hit = true;
            }
        }

        //cb->m_usedNode.insert(current);

        if (Intersect(ray, cb, current, idBitMask))
            hit = true;
    }
...
void CSceneCollisionManager::getPickedNodeBB(ISceneNode* root,
		G3D::Ray& ray, s32 bits, bool noDebugObjects,
				...
				core::triangle3df & outBestTriangle)
{
	...
	for (; it != children.end(); ++it)
	{
		...
		if (selector && current->isVisible() &&
			...
			(bits==0 || (bits != 0 && (current->getID() & bits))))
		{
            if (selector->IntersectWithRay(ray, outBestDistanceSquared))
            {
                G3D::Vector3 pos = ray.origin() + outBestDistanceSquared * ray.direction();
                outBestDistanceSquared = outBestDistanceSquared;
                outBestNode = current;
                outBestCollisionPoint = irr::core::vector3df(pos.x, pos.y, pos.z);
	...
	ICameraSceneNode* camera, s32 idBitMask, bool noDebugObjects)
{
    ...
    //
	//return getSceneNodeFromRayBB(G3D::Ray(start, end), idBitMask, noDebugObjects);
}


//! Finds the collision point of a line and lots of triangles, if there is one.
bool CSceneCollisionManager::getCollisionPoint(G3D::Ray& ray,
		...
		ISceneNode*& outNode)
{
    ...
    //// get world to object space transform
    ////core::matrix4 invMat;
    ////if (!selector->getAbolutTransformation(0).getInverse(invMat))
    ////    return false;
    ////
    ////// transform vector from world space to object space
    ////G3D::Ray& ray(line);
    ...
    ////mat.setbyproduct(selector->getSceneNodeForTriangle(0)->getAbsoluteTransformation(), invMat);
    //
    ...
    //
	//const core::vector3df rayvect = ray.getVector().normalize();
	//core::vector3df intersection;
	//f32 nearest = FLT_MAX;
	//bool found = false;
    ...
    //
    ////bool selected = selector->getSceneNodeForTriangle(0)->isDebugDataVisible();
    //
	//for (s32 i=0; i<cnt; ++i)
	//{
	//	core::triangle3df triangle = Triangles[i];
    //
    //    //triangle.pointA = mat * triangle.pointA;
    //    //triangle.pointB = mat * triangle.pointB;
    //    //triangle.pointC = mat * triangle.pointC;
    //
	...
	//		continue;
    //
	//	if (triangle.getIntersectionWithLine(ray.start, rayvect, intersection))
	//	{
    ...
    //            //outIntersection = selector->getSceneNodeForTriangle(0)->getAbsoluteTransformation() * intersection;
