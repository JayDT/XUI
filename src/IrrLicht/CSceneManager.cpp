    ...
    IRR_XML_FORMAT_SCENE(L"irr_scene"), IRR_XML_FORMAT_NODE(L"node"), IRR_XML_FORMAT_NODE_ATTR_TYPE(L"type")
{
    ...
    mRequestToggleFreezeCulling = false;
    mFreezeCulling = false;
    mRegenerateCulling = true;
    Terrain = nullptr;
...
//! gets an animateable mesh. loads it if needed. returned pointer must not be dropped.
IAnimatedMesh* CSceneManager::getMesh(io::IReadFile* file, bool useCache)
{
    if (!file)
        return 0;
    ...
    case ESNRP_TERRAIN:
        if (!isCulled(node))
        {
            if (Terrain == nullptr)
                ...
                throw std::runtime_error("invalid terrain count");
            taken = 1;
        }
        break;
    case ESNRP_SOLID:
...
__forceinline u32 getMSTimeDiff(u32 oldMSTime, u32 newMSTime)
{
    // getMSTime() have limited data range and this is case when it overflow in this tick
    if (oldMSTime > newMSTime)
        return (static_cast<u32>(0xFFFFFFFF) - oldMSTime) + newMSTime;
    else
...
void CSceneManager::drawAll()
{
    ...
    */
    ...
    if (!mFreezeCulling)
    {
        ...
        mRegenerateCulling = false;

        if (getMSTimeDiff(mNextCullingTime, os::Timer::getRealTime()) > 100)
        {
            mNextCullingTime = os::Timer::getRealTime();
            mRegenerateCulling = true;
        }
    ...
    {
        if (mRequestToggleFreezeCulling && getActiveCamera())
            getActiveCamera()->setFarValue(27777.f);
        mFreezeCulling = mRequestToggleFreezeCulling;
    }
    ...
    //render camera scenes
    {
        ...
        if (!mFreezeCulling && mRegenerateCulling)
    ...
    if (Terrain)
    {
        CurrentRendertime = ESNRP_TERRAIN;
        Terrain->render();
    }
