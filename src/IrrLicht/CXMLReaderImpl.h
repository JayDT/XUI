...
public:
	...
	CXMLReaderImpl(IFileReadCallBack* callback, bool deleteCallBack = true)
		: IgnoreWhitespaceText(true), TextData(0), P(0), TextBegin(0), TextEnd(0), TextSize(0), CurrentNodeType(EXN_NONE),
...
private:
	...
	void parseOpeningXMLElement()
	{
		...
		// find end of element
		while(*P != L'>' && !isWhiteSpace(*P) && P <= TextEnd)
			++P;
		...
		while(*P != L'>' && P <= TextEnd)
		{
			...
			else
			{
				if (*P != L'/')
				{
					...
					while(!isWhiteSpace(*P) && *P != L'=' && P <= TextEnd)
						++P;
	...
	*/
	...
	void convertTextData(src_char_type* source, char* pointerToStore, int sizeWithoutHeader)
	{
		...
		if (sizeof(src_char_type) == sizeof(char_type))
		{
            ...
            TextEnd = (char_type*)(source + TextSize);
        }
		else
		{
