...
void CD3D9Driver::createMaterialRenderers()
{
    ...
    addAndDropMaterialRenderer(new CD3D9MaterialRenderer_RAW(pID3DDevice, this));
}
...
bool CD3D9Driver::initDriver(HWND hwnd, bool pureSoftware)
{
	if (!pID3D)
	{
		...
		//typedef IDirect3D9Ex * (__stdcall *D3DCREATETYPE)(UINT);
        //D3DCREATETYPE d3dCreate = (D3DCREATETYPE)GetProcAddress(D3DLibrary, "Direct3DCreate9Ex"); // DirectX 9c

        typedef IDirect3D9 * (__stdcall *D3DCREATETYPE)(UINT);
        D3DCREATETYPE d3dCreate = (D3DCREATETYPE) GetProcAddress(D3DLibrary, "Direct3DCreate9"); // DirectX 9
        ...
        //HRESULT hr = Direct3DCreate9Ex(D3D_SDK_VERSION, &pID3D);
		...
		const SExposedVideoData& videoData, core::rect<s32>* sourceRect)
{
	...
	if (FAILED(hr))
	{
        ...
        //printf("[DIRECT3D9] Error: %s error description: %s\n",
...
//#endif
    ...
    ShaderHLSL* gpuProgram = new ShaderHLSL(this, EST_HIGH_LEVEL_SHADER);
    if (vertexShader)
    {
        ...
        if (FAILED(hr))
        {
            if (pBufferErrors)
            {
                ...
                printf("[HLSL SHADER] Vertex Shader Compile Error: Not file found \r\n");

            delete gpuProgram;
            return nullptr;
            //throw std::runtime_error("HLSL SHADER: failed");
        }

        // Create the vertex shader
        hr = pID3DDevice->CreateVertexShader((DWORD*)pCode->GetBufferPointer(), &gpuProgram->m_pVertexShader);
    ...
    if (fragmentShader)
    {
        ...
        // Create the vertex shader
        hr = pID3DDevice->CreatePixelShader((DWORD*)pCode->GetBufferPointer(), &gpuProgram->m_pPixelShader);
...
{
    ShaderHLSL* hlsl = (ShaderHLSL*)getActiveGPUProgram();
...
void CD3D9Driver::useShader(IShader* gpuProgram)
{
    if (gpuProgram)
    {
        if (gpuProgram->getShaderVersion() == ESV_HLSL_HIGH_LEVEL)
        {
            ...
            pID3DDevice->SetPixelShader(hlsl->m_pPixelShader);
            //pID3DDevice->SetSoftwareVertexProcessing(false);
        }
        else
        {
        }
    ...
    {
        //pID3DDevice->SetSoftwareVertexProcessing(true);
        pID3DDevice->SetVertexShader(nullptr);
        pID3DDevice->SetPixelShader(nullptr);
    }
}

void CD3D9Driver::buildShaderVariableDescriptor(IShader* _gpuProgram)
{
    ShaderHLSL* gpuProgram = (ShaderHLSL*)_gpuProgram;
    //ShaderVariableDescriptor ui;
    //
    //ui.m_location = 1; // Stream index;
    //ui.m_length = 1; // dynamic
    //ui.m_variableType = D3DXPT_FLOAT;
    //ui.m_class = D3DXPC_MATRIX_ROWS;
    //ui.m_name = "WVP";
    //ui.m_size = sizeof(core::matrix4);
    //ui.m_type = ESVT_INPUT_STREAM;
    //ui.m_shaderIndex = 0;
    //
    //gpuProgram->AddShaderVariable(&ui);
    //
    //gpuProgram->LinkShaderVariable("WVP", irr::video::EGVAT_VIEW_MATRIX);
...
bool CD3D9Driver::updateIndexHardwareBuffer(CD3D9HardwareBuffer *hwBuffer)
{
	if (!hwBuffer)
		return false;
...
IHardwareBuffer *CD3D9Driver::createHardwareBuffer(const scene::IMeshBuffer* mb)
{
...
void CD3D9Driver::deleteHardwareBuffer(IHardwareBuffer *_HWBuffer)
{
	if (!_HWBuffer)
		return;
...
bool CD3D9Driver::reset()
{
    ...
    for ( auto& iTex : Textures )
    {
		...
		{
			IDirect3DBaseTexture9* tex = ((CD3D9Texture*)(iTex.second))->getDX9Texture();
			if (tex)
				tex->Release();
		}
	}
	...
	{
		if ( iTex.second->isRenderTarget())
			((CD3D9Texture*)(iTex.second))->createRenderTarget();
	}
	...
	for (i=1; i<DepthBuffers.size(); ++i)
	{
		for (auto& iTex : Textures)
		{
			...
			// so take first one
			if (((CD3D9Texture*)(iTex.second))->DepthSurface==DepthBuffers[i])
			{
				((CD3D9Texture*)(iTex.second))->Texture->GetLevelDesc(0,&desc2);
				break;
			}
		}
