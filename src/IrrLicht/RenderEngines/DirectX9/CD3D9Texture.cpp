...
//! lock function
void* CD3D9Texture::lock(E_TEXTURE_LOCK_MODE mode, u32 mipmapLevel, u32 arraySlice)
{
	if (!Texture)
		return 0;
...
void CD3D9Texture::regenerateMipMapLevels(void* mipmapData)
{
	if (mipmapData)
	{
        ...
        do
		{
            ...
            //u32 sizeOfData = size.getArea()*getPitch() / TextureSize.Width;
            //switch (ColorFormat)
            //{
            //    case ECF_RGBA_S3TC_DXT1:
            //        sizeOfData = max(8, sizeOfData);
            //        break;
            //    case ECF_RGBA_S3TC_DXT3:
            //    case ECF_RGBA_S3TC_DXT5:
            //        sizeOfData = max(16, sizeOfData);
            //        break;
            //}
            //_IRR_DEBUG_BREAK_IF(sizeOfData != imageSize);
            memcpy(miplr.pBits, target, imageSize);
