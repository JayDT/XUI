	...
	ReadOnlyLock(false), KeepImage(true)
{
    ...
    switch (origImage->getColorFormat())
    {
        case ECF_RGBA8:
    ...
    //if (Image->getDimension().Height != Image->getDimension().Width)
    //    mipmapData = nullptr;
...
void COpenGLTexture::uploadTexture(bool newTexture, void* mipmapData, u32 level)
{
    ...
    switch (InternalFormat)
    {
        ...
        case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
            Driver->pGlCompressedTexImage2D(GL_TEXTURE_2D, level, InternalFormat, image->getDimension().Width, image->getDimension().Height, 0, image->getImageDataSizeInBytes(), source);
            break;
        default:
        {
            if (newTexture)
...
void COpenGLTexture::regenerateMipMapLevels(void* mipmapData)
{
	...
	do
	{
        ...
        else
        {
            ...
            //glTexImage2D(GL_TEXTURE_2D, i, GL_RGBA8, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, target);
            switch (InternalFormat)
            {
                case GL_COMPRESSED_RGBA_S3TC_DXT1_EXT:
                case GL_COMPRESSED_RGBA_S3TC_DXT3_EXT:
                case GL_COMPRESSED_RGBA_S3TC_DXT5_EXT:
                {
                    ...
                    Driver->pGlCompressedTexImage2D(GL_TEXTURE_2D, level, InternalFormat, width, height, 0, nSize /*imageSize*/, target);
                    break;
                }
                default:
                {
                    glTexImage2D(GL_TEXTURE_2D, level, InternalFormat, width, height, 0, PixelFormat, PixelType, target);
                    break;
                }
            }
