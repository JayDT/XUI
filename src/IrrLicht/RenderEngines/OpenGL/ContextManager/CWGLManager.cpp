...
bool CWGLManager::initialize(const SIrrlichtCreationParameters& params, const SExposedVideoData& videodata)
{
	...
	PIXELFORMATDESCRIPTOR tmp_pfd = {
		...
		BYTE(Params.Stencilbuffer ? 8 : 0),        // Stencil Buffer Depth
...
{
	if (PrimaryContext.OpenGLWin32.HDc)
		ReleaseDC((HWND)PrimaryContext.OpenGLWin32.HWnd, (HDC)PrimaryContext.OpenGLWin32.HDc);
	if (PrimaryContext.OpenGLWin32.HDc && PrimaryContext.OpenGLWin32.HDc == PrimaryContext.OpenGLWin32.HDc)
...
{
	HDC HDc = (HDC)PrimaryContext.OpenGLWin32.HDc;
...
bool CWGLManager::generateContext()
{
	...
	// set exposed data
    PrimaryContext.OpenGLWin32.HRc = hrc;
	if (!PrimaryContext.OpenGLWin32.HRc)
		PrimaryContext.OpenGLWin32.HRc= PrimaryContext.OpenGLWin32.HRc;
...
{
	return SwapBuffers((HDC)PrimaryContext.OpenGLWin32.HDc) == TRUE;
}
