...
namespace scene
{
	...
	template <class T>
    class IRRLICHT_API CMeshBuffer : public IMeshBuffer
	{
	public:
        ...
        //CMeshBuffer(const CMeshBuffer&) = default;
        //CMeshBuffer& operator=(const CMeshBuffer&) = default;
        ...
        virtual video::S3DVertex& operator[](int idx)
        {
            return Vertices[idx];
        }
    ...
    template <class T>
    class IRRLICHT_API CMeshBufferEx : public CMeshBuffer<T>
    {
        struct CMeshVertexBufferEx
        {
            CMeshVertexBufferEx()
            {
                m_indicesStart = 0;
        ...
        virtual ~CMeshBufferEx()
        {
            for ( u32 i = 0; i != SubBuffer.size(); ++i )
                delete SubBuffer[i];
        }

        virtual video::IShader* GetGPUProgram() const
        {
            return m_gpuProgram;
        }
        ...
        virtual s32 GetVertexRangeStart() const
        {
            return !SubBuffer.empty() ? SubBuffer[ActiveSubBuffer]->m_vectiesStart : 0;
        }
        ...
        {
            return !SubBuffer.empty() ? SubBuffer[ActiveSubBuffer]->m_indicesStart : 0;
        }
