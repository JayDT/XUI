...
struct IGUIFontCache
{
    virtual ~IGUIFontCache() {}
    virtual scene::IMeshBuffer& GetBuffer() = 0;
    virtual core::stringw& GetText() = 0;
};

//! Font interface.
class IRRLICHT_API IGUIFont : public virtual IReferenceCounted
{
public:
	...
	virtual void draw(const core::stringw& text, const core::rect<s32>& position,
		...
		const core::rect<s32>* clip=0, IGUIFontCache* cache = nullptr) = 0;
    ...
    virtual u32 BreakText(const wchar_t* text, float maxWidth, float* measuredWith = nullptr) const = 0;
    ...
    virtual core::dimension2d<u32>  MeasureText(const wchar_t* text, irr::core::recti& bounds) = 0;
    //
    // Summary:
    //     Measures the specified text.
    //
    ...
    //     This will return the vertical measure if this is vertical text, in which case
    //     the returned value should be treated has a height instead of a width.
    virtual core::dimension2d<u32>  MeasureText(const wchar_t* text) = 0;
    //
    // Summary:
    ...
    // Parameters:
    //   buffer:
    //     The pointer to a region holding text encoded using the encoding specified in
    //     SkiaSharp.SKPaint.TextEncoding format.
    //
    //   length:
    ...
    //     the returned value should be treated has a height instead of a width.
    //     ///
    //     The buffer parameter is a pointer to a region in memory that contains text encoded
    //     in the SkiaSharp.SKPaint.TextEncoding format. This only consumes up to length
    //     bytes from the buffer.
    //     ///
    virtual core::dimension2d<u32>  MeasureText(const wchar_t* text, u32 length) = 0;
