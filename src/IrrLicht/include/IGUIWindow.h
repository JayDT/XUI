...
namespace gui
{
	...
	*/
	...
	{
        friend class Forms::IForm;

        Forms::IForm* m_formContent;

        void SetParentForm(Forms::IForm* f)
        {
            m_formContent = f;
        }
	public:
		...
		IGUIWindow(IGUIEnvironment* environment, IGUIElement* parent, s32 id, core::rect<s32> rectangle)
			: IGUIElement(EGUIET_WINDOW, environment, parent, id, rectangle), m_formContent(nullptr) {}

        virtual ~IGUIWindow()
		{
            m_formContent = nullptr;
		}
