...
namespace scene
{
    ...
    enum E_COLLISON_ENABLE_FLAGS
    {
        ECEF_ENABLE_NO_VISIBLE  = 0x00000001,
        ECEF_ENABLE_DEBUG_SCENE = 0x00000002,
    };

    struct IIntersectionCallback
    {
        std::set<ISceneNode*> m_usedNode;
        virtual u32 getEnableFlags() const = 0;
        virtual bool hasHit() const = 0;
        virtual float LastHitDistance() const = 0;
        virtual void DoHit(float& distance, irr::scene::ISceneNode* node) = 0;
    };

    struct StandardIntersectionCallback : IIntersectionCallback
    {
        bool m_hit;
        float m_hitDistance;
        u32 m_enableFlags;
        irr::scene::ISceneNode* m_hitNode;
        G3D::Ray m_ray;

        StandardIntersectionCallback() : m_hit(false), m_hitDistance(FLT_MAX), m_enableFlags(0), m_hitNode(nullptr)
        {

        }

        // E_Intersection_Enable_Flags
        StandardIntersectionCallback(float maxDist) : m_hit(false), m_hitDistance(maxDist), m_enableFlags(0), m_hitNode(nullptr)
        {

        }

        virtual u32 getEnableFlags() const { return  m_enableFlags; }
        virtual bool hasHit() const { return m_hit; }
        virtual float LastHitDistance() const { return m_hitDistance; }
        virtual void DoHit(float& distance, irr::scene::ISceneNode* node)
        {
        ...
        virtual irr::core::vector3df GetHitPosition(float distanceOffset = 0.f)
        {
            G3D::Vector3 pos = m_ray.origin() + (m_hitDistance + distanceOffset) * m_ray.direction();
            return irr::core::vector3df(pos.x, pos.y, pos.z);
        }
    };

    //! The Scene Collision Manager provides methods for performing collision tests and picking on scene nodes.
    class IRRLICHT_API ISceneCollisionManager : public virtual IReferenceCounted
    {
    public:

        //! Generic Collision Test Method
        virtual bool Intersect(G3D::Ray const& ray, scene::IIntersectionCallback* cb, ISceneNode * collisionRootNode = 0, s32 idBitMask = 0) = 0;
        ...
        virtual bool getCollisionPoint(G3D::Ray& ray,
        ...
        virtual ISceneNode* getSceneNodeFromRayBB(G3D::Ray& ray,
                            s32 idBitMask=0, bool bNoDebugObjects=false, ISceneNode* root=0) =0;
