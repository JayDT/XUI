...
namespace scene
{
	...
	public:
		...
		struct SWeight
		{
			...
			//! Index of the vertex
			u16 vertex_id; //Store global ID here
		...
		struct SJoint
		{
            ...
            void getValue(T& ret, core::array<AT> &array, float frame, s32 rangeStart, s32 rangeEnd)
            {
                ...
                //{
                //    //check this hint
                //    if (LastIndex > 0 && PositionKeys[LastIndex].frame > frame && PositionKeys[LastIndex - 1].frame < frame)
                //        pos = positionHint;
                //    else if (positionHint + 1 < (int32)PositionKeys.size())
                //    {
                ...
                //if (pos == -1)
                //{
                //    for (uint i = 0; i < PositionKeys.size(); ++i)
                //    {
                //        if (PositionKeys[i].frame > frame && PositionKeys[i].frame <= animEndTime && PositionKeys[i].frame >= animStartTime) //Keys should to be sorted by frame
                //        {
                //            pos = i;
                //            LastIndex = i;
                //            break;
			...
			bool GlobalSkinningSpace : 1;
            bool isTransportFormed : 1;
