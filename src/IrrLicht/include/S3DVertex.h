...
*/
struct IRRLICHT_API S3DVertex2TCoords : public S3DVertex
{
    //! default constructor
...
/** Usually used for tangent space normal mapping. */
struct IRRLICHT_API S3DVertexTangents : public S3DVertex
{
    //! default constructor
...
struct S3DSkinningVertex : public S3DVertex2TCoords
{
    ...
    //! constructor with all values
    S3DSkinningVertex(const core::vector3df& pos, const core::vector3df& normal, const video::SColor& color,
        const core::vector2d<f32>& tcoords, const core::vector2d<f32>& tcoords2, const u8* bones, const u8* weights)
        : video::S3DVertex2TCoords(pos, normal, color, tcoords, tcoords2)
    {
        for (u8 i = 0; i != 4; ++i)
        {
            Bones[i]   = bones[i];
