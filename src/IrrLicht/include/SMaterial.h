...
namespace video
{
		...
		E_BLEND_OPERATION* blendOp = nullptr, E_BLEND_OPERATION* blendOpAlpha = nullptr)
	{
		...
		if (blendOp)
			*blendOp = E_BLEND_OPERATION( (state & 0x0F000000) >> 24 );
		if (srcFactAlpha)
			*srcFactAlpha = E_BLEND_FACTOR( (state & 0x00F00000) >> 20 );
		if (dstFactAlpha)
			*dstFactAlpha = E_BLEND_FACTOR( (state & 0x000F0000) >> 16 );
	...
	*/
	...
	public:
		...
		SMaterial()
		: m_textureRefMgr(nullptr), MaterialType(EMT_SOLID), AmbientColor(255,255,255,255), DiffuseColor(255,255,255,255), AnimateColor(255, 255, 255, 255), Transparency(255, 255, 255, 255),
