...
class aabbox3d
{
    public:
        ...
        void getEdgesIndcies(u16*& indicies, u16& size) const
        {
            ...
            indicies = sIndicies;
            size = sizeof(sIndicies) / sizeof(u16);
        }
        ...
        \return True if there is an intersection with the other box,
        otherwise false. */
        bool intersects2DWithBox(const aabbox3d<T>& other) const
        {
            return (MinEdge.X <= other.MaxEdge.X && MinEdge.Z <= other.MaxEdge.Z &&
                MaxEdge.X >= other.MinEdge.X && MaxEdge.Z >= other.MinEdge.Z);
        }

        bool Overlapping2D(const aabbox3d<T>& other) const
        {
            auto extent = getExtent();
            auto center = getCenter();

            auto extent2 = other.getExtent();
            auto center2 = other.getCenter();

            return ::fabs(center.X - center2.X) <= (extent.X + extent2.X) &&
                ::fabs(center.Z - center2.Z) <= (extent.Z + extent2.Z);
        }
