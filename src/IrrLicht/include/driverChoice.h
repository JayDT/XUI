...
{
	const char* const names[] = {"NullDriver","Software Renderer","Burning's Video","Direct3D 8.1","Direct3D 9.0c", "Direct3D 11", "OpenGL 1.x/2.x/3.x"};
	printf("Please select the driver you want:\n");
	irr::u32 i=0;
	for (i=irr::video::EDT_COUNT; i>0; --i)
	{
        if (allDrivers || (irr::isDriverSupported(irr::video::E_DRIVER_TYPE(i - 1))))
