...
public:
	...
	array()
		: data(0), allocated(0), used(0),
			strategy(ALLOC_STRATEGY_HYBRID), free_when_destroyed(true), is_sorted(true)
	{
	}
	...
	*/
	...
	void setAllocStrategy ( eAllocStrategy newStrategy = ALLOC_STRATEGY_HYBRID )
	{
		...
		if (used + 1 > allocated)
		{
			...
			switch ( strategy )
			{
				case ALLOC_STRATEGY_DOUBLE:
					...
					break;
                case ALLOC_STRATEGY_SQRT:
                    newAlloc = used + 1 + (allocated < 5 ? 5 : used >> 1);
                    break;
                case ALLOC_STRATEGY_HYBRID:
                    newAlloc = used + 1 + (allocated < 500 ?
                        (allocated < 5 ? 5 : used >> 1) : used >> 2);
                    break;
                default:
