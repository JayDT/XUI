...
void translate_string(const std::wstring &s, const std::wstring &textdomain,
	...
	while (i < s.length()) {
		...
		if (parts[0] == L"E") {
			...
			if (arg_number >= 10) {
                std::cout << "Ignoring too many arguments to translation" << std::endl;
				std::wstring arg;
            ...
            std::cout << "Ignoring escape sequence '" << wide_to_narrow(escape_sequence) << "' in translation" << std::endl;
		}
	}
	...
	while (j < toutput.length()) {
		...
		if (0 <= arg_index && (size_t)arg_index < args.size()) {
            ...
            std::cout << "Ignoring out-of-bounds argument escape sequence in translation" << std::endl;
		}
	}
	res = result.str();
}
